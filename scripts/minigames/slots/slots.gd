...
const SYMBOLS : Array = [
	...
	"res://assets/minigames/sprites/slots/dia_blu.png",
...
# Each reel has (VISIBLE_ROWS + 2) slots.
# Each slot = {tex: TextureRect, lbl: Label, bg: ColorRect}
# We only swap .texture / .text — no queue_free ever during spin
var reel_slots   : Array = []   # reel_slots[col][slot_idx] = dict
var reel_windows : Array = []
var stop_buttons : Array = []
var result_label : Label

# Pre-loaded textures — load once at startup
var _textures    : Array = []
...
func _build_ui() -> void:
	...
	for col in range(COL_COUNT):
		# Clip window — use SubViewportContainer-style clipping via Panel
		var win := Panel.new()
		...
		reel_windows.append(win)

		# Pre-build VISIBLE_ROWS+2 slots — never destroyed, just repositioned
		var slots : Array = []
		for s in range(VISIBLE_ROWS + 2):
			var slot_dict := _build_slot_node(win)
...
# Build one slot node dict — root Control + either TextureRect or Label inside
...
# Update a slot's content by swapping texture/text only — no node creation
...
func _generate_reels() -> void:
	...
	for _i in range(COL_COUNT):
		...
		# Guarantee target appears at least once so it's always catchable
...
func _start_spin() -> void:
	...
	reel_speeds  = [speed * 1.2, speed * 1.8, speed * 2.5]  # slow / medium / fast
...
func _on_stop(col: int) -> void:
	...
	# Snap to nearest symbol boundary
	...
	if hit:
		...
		# Check if all columns stopped and all are stars
		if stopped.all(func(v): return v):
			spinning = false
			await get_tree().create_timer(0.4).timeout
			_check_result()
	else:
		# Wrong symbol — instant fail, reset immediately
		stop_buttons[col].text = "✗"
		_style_stop_btn(stop_buttons[col], false)  # red
		AudioManager.play_sfx(AudioManager.SFX.WRONG)
		spinning = false

		attempts += 1
		if result_label:
			result_label.text = "Wrong symbol! Attempt "
...
func _check_result() -> void:
	# Only called when all 3 columns are stopped on ★
