...
func _ready():
	var current_index = GameManager.current_article_index
	
	load_article_data(current_index)
...
func _on_accept_pressed():
	...
	var game_path = GameManager.get_current_game_path()
	
	if game_path != "":
		get_tree().change_scene_to_file(game_path)
	else:
		print("❌ Error: No game path found!")
